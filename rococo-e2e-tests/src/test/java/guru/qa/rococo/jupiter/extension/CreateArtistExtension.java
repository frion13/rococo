...
import guru.qa.rococo.data.repository.ArtistRepositorySpringJdbc;
...
public class CreateArtistExtension implements BeforeEachCallback, ParameterResolver {
    ...
    private ArtistEntity createArtistForTest(GenerateArtist annotation) {
        ...
        String name = annotation.name().isEmpty() ? RandomDataUtils.randomUsername() : annotation.name();
        String biography = annotation.biography().isEmpty() ? RandomDataUtils.randomSentence(15) : annotation.biography();
